...
export class ChatView extends ItemView {
  ...
  async onOpen(): Promise<void> {
    ...
    const saveBtn = toolbar.createEl('button', { cls: 'oac-save-btn', text: 'Save as Note' });
  ...
  private async handleSend(): Promise<void> {
    ...
    const assistantHandle = appendMessage(
      ...
      'assistant',
      this.app,
      (selectedText, parentMessage) => this.openElaborateView(selectedText, parentMessage, true),
      (selectedText, parentMessage) => this.openElaborateView(selectedText, parentMessage, false)
    );
    this.scrollToBottom();
    ...
    try {
      const context = await buildContext(this.app, this.plugin.settings.contextWindowLines);
      ...
      new Notice(`AI Chat error: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
  ...
  private async handleSave(): Promise<void> {
    ...
    const fileName = `${folder}/Chat ${timestamp}.md`;
  ...
  private openElaborateView(selectedText: string, parentMessage: string, autoSend: boolean): void {
    const prefill = autoSend
      ? `Please elaborate on the following:\n\n${selectedText}`
      : `Regarding: ${selectedText}\n\n`;

    this.plugin.openElaborateView({ prefillText: prefill, systemContext: parentMessage, autoSend });
  }
}
